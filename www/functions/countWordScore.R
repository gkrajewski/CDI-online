countScore <- function(type){
  
  wordAnswers <- answers[answers$type == "word",]
  wordInputType <- typeUniqueSettings[typeUniqueSettings$type == "word", "input_type"]
  
  if (form == "wg"){
    
    answersStrings <- wordAnswers[wordAnswers$answer_type == "manyCheckboxGroups", "answer"]
    ...
    answersInt <- na.omit(wordAnswers[wordAnswers$answer_type == "oneCheckboxGroup", "answer"])
